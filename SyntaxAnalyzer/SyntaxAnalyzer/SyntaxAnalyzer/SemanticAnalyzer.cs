...
namespace SyntaxAnalyzer
{
    class SemanticAnalyzer
    {
		...
		private List<string> expression = new List<string>();
       		private List<string> operations =  new List<string> { "<>", "=", "<", "<=", ">", ">=", "+", "-", "or", "*", "(", ")", "and", "mult", "div", "!" };
       ...
       public void StartSemanticAnalyzer()
        ...
        public bool CheckAssignment()
		{
            foreach (var item in operationsAssignments)
            {
                ...
                for (int i = 1; i < itemArr.Length; i++)
                {
					...
					{
                        if( ((itemArr[i] == "true" || itemArr[i] == "false") && type != "boolean" ) || (_numberWithType[itemArr[i]] != type))
                        {
                            return false;
						}
					}
        ...
        public bool CheckDiv()
		{
            foreach (var item in operationsAssignments)
            {
                ...
                for (int i = 1; i < itemArr.Length; i++)
                {
                    if (itemArr[i] == "/" && (type == "integer" || type == "boolean"))
                    {
                        return false;
                    }
                }
